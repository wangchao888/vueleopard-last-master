<template>
  <div>
    ...
    <Menu v-if="!sidebar" ref="sideMenu" :active-name="$route.path" :theme="menuTheme" :open-names="openedSubmenuArr"
      ...
      <template v-for="item in permission_routers" v-if="item.hidden!=true && item.path.indexOf(defaultIndex)!=-1">
  ...
  <template v-for="(item, index) in permission_routers" v-if="item.hidden!=true && item.path.indexOf(defaultIndex)!=-1">
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      defaultIndex(){
        return this.$store.state.app.defaultIndex;
      },
      openedSubmenuArr(){
